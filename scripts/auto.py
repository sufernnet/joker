...
class LiveStreamFetcher:
    ...
    def classify_channels(self):
        ...
        # 需要查找的频道列表（包括CCTV5+）
        target_channels = [f'CCTV{i}' for i in range(1, 18)] + ['CCTV5+']
        # 对应的标准名称
        standard_names = {f'CCTV{i}': f'CCTV-{i}' for i in range(1, 18)}
        standard_names['CCTV5+'] = 'CCTV-5+'
        ...
        if content:
            ...
            for i, line in enumerate(lines):
                ...
                if '#EXTINF:' in line and 'group-title=' in line:
                    # 检查是否是央视分组（支持多种可能的分组名称）
                    if '央视卫视' in line or '央视' in line:
                        in_cctv_group = True
                        # 提取频道名称
                        match = re.search(r',(.+)$', line)
                        if match and i + 1 < len(lines):
                            ...
                            # 匹配CCTV1~CCTV17和CCTV5+
                            for target in target_channels:
                                # 使用正则匹配，支持多种格式如 CCTV5, CCTV-5, CCTV5体育, CCTV5+等
                                if target == 'CCTV5+':
                                    pattern = r'CCTV[-]?5\+'
                                else:
                                    num = target.replace('CCTV', '')
                                    pattern = rf'CCTV[-]?{num}(?!\+)'  # 否定前瞻，避免匹配到CCTV5+
                                
                                if re.search(pattern, name, re.IGNORECASE):
                                    standard_name = standard_names[target]
            ...
            # 按顺序添加到央视分组（CCTV-1 到 CCTV-17，然后是 CCTV-5+）
            for i in range(1, 18):
                channel_name = f'CCTV-{i}'
            ...
            # 添加 CCTV-5+
            if 'CCTV-5+' in cctv_channels_found:
                groups['央视'].append(('CCTV-5+', cctv_channels_found['CCTV-5+']))
                logging.info(f"  CCTV-5+: 已添加")
            else:
                logging.warning(f"  CCTV-5+: 未找到链接")
        else:
