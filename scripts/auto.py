...
class LiveStreamFetcher:
    ...
    def classify_channels(self):
        ...
        # ========== 1. 央视分组：直接从指定订阅源拉取 央视卫视 分组 ==========
        cctv_source_url = "https://abc.sufern001.workers.dev"
        cctv_channels = {}

        try:
            logging.info(f"从 {cctv_source_url} 获取央视频道...")
            content = self.fetch_m3u_content(cctv_source_url)
            if content:
                channels_with_group = self.parse_m3u_with_group(content)
                for group, name, url in channels_with_group:
                    if group == "央视卫视":
                        clean_name = re.sub(r'[\[\(].*?[\]\)]', '', name).strip()
                        # 统一标准名称
                        if re.search(r'CCTV[-]?5\+', clean_name, re.I):
                            std_name = "CCTV-5+高清"
                        elif re.search(r'CCTV[-]?(\d+)', clean_name):
                            num = re.search(r'(\d+)', clean_name).group(1)
                            std_name = f"CCTV-{num}高清"
                        else:
                            continue

                        if std_name not in cctv_channels:
                            cctv_channels[std_name] = url

                # 按顺序写入：CCTV-1~17高清 + CCTV-5+高清
                cctv_order = [f"CCTV-{i}高清" for i in range(1, 18)] + ["CCTV-5+高清"]
                for std in cctv_order:
                    if std in cctv_channels:
                        groups['央视'].append((std, cctv_channels[std]))
        except:
            pass
        ...
        digital_mapping = {
            '第一剧场': 'CCTV-第一剧场', '风云剧场': 'CCTV-风云剧场', '风云音乐': 'CCTV-风云音乐',
            '风云足球': 'CCTV-风云足球', '怀旧剧场': 'CCTV-怀旧剧场', '央视文化': 'CCTV-央视文化',
        ...
        for name, url in self.channels.items():
            if '凤凰' in name:
                ...
                if any(kw in name for kw in
                       ['香港', 'TVB', '無綫', '无线', '千禧', '翡翠', '明珠', '星影', '爆谷', 'HOY', 'NOW', 'HBO', 'CABLE', '港台',
                        'RTHK', '亚洲电视', 'ATV', 'VIU']):
        ...
        if hasattr(self, 'hk_tw_sources'):
            for name, url in self.hk_tw_sources.items():
                if any(kw in name for kw in
                       ['台湾', '台视', '寰宇', '中视', '华视', '民视', '三立', '公视', '三立', '东森', '中天', 'TVBS', '年代', '非凡',
                        '八大', '纬来', '客家', '原住民']):
