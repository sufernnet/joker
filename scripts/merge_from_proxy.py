#!/usr/bin/env python3
"""
M3U文件合并脚本 - 优化EPG版本
1. 下载BB.m3u（包含EPG信息）
2. 从Cloudflare代理获取内容
3. 提取JULI频道，分组改为HK，按指定顺序排列
4. 提取4gtv前30个直播，分组改为TW，过滤指定频道
5. 合并生成CC.m3u，确保EPG链接有效
北京时间每天6:00、18:00自动运行
"""

import requests
import re
import os
import time
from datetime import datetime

# 配置
BB_URL = "https://raw.githubusercontent.com/sufernnet/joker/main/BB.m3u"
CLOUDFLARE_PROXY = "https://smt-proxy.sufern001.workers.dev/"
OUTPUT_FILE = "CC.m3u"

# 需要过滤掉的TW频道关键词（不区分大小写）
BLACKLIST_TW = [
    "Bloomberg TV", "Bloomberg", "SBN全球财经台", "SBN财经",
    "FRANCE24英文台", "FRANCE24", "半岛国际新闻台", "半岛国际",
    "NHK world-japan", "NHK world", "NHK", "CNBC Asia", "CNBC"
]

# HK频道优先顺序（按这个顺序排列在最前面）
HK_PRIORITY_ORDER = [
    "凤凰中文", "凤凰资讯", "凤凰香港",
    "NOW新闻台", "NOW星影", "NOW爆谷"
]

def log(msg):
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {msg}")

def download_bb_m3u():
    """下载BB.m3u并提取EPG"""
    try:
        log("下载BB.m3u...")
        response = requests.get(BB_URL, timeout=10)
        response.raise_for_status()
        
        bb_content = response.text
        log(f"✅ BB.m3u下载成功 ({len(bb_content)} 字符)")
        
        # 提取EPG信息
        epg_match = re.search(r'url-tvg="([^"]+)"', bb_content)
        epg_url = epg_match.group(1) if epg_match else None
        
        if epg_url:
            log(f"✅ 提取到EPG: {epg_url}")
        
        return bb_content, epg_url
        
    except Exception as e:
        log(f"❌ BB.m3u下载失败: {e}")
        return None, None

def get_content_from_proxy():
    """从Cloudflare代理获取内容"""
    try:
        log("从Cloudflare代理获取内容...")
        headers = {
            'User-Agent': 'Mozilla/5.0',
            'Accept': '*/*'
        }
        
        response = requests.get(CLOUDFLARE_PROXY, headers=headers, timeout=15)
        
        if response.status_code == 200:
            content = response.text
            
            # 如果是HTML，尝试提取M3U内容
            if '<html' in content.lower():
                m3u_match = re.search(r'(#EXTM3U.*?)(?:</pre>|</code>|$)', content, re.DOTALL)
                if m3u_match:
                    content = m3u_match.group(1).strip()
                    log("✅ 从HTML提取到M3U内容")
            
            if content and content.strip():
                log(f"✅ 获取到内容 ({len(content)} 字符)")
                return content
        else:
            log(f"❌ 代理返回错误: {response.status_code}")
            
    except Exception as e:
        log(f"❌ 代理访问失败: {e}")
    
    return None

def extract_hk_channels(content):
    """提取JULI频道，分组改为HK"""
    if not content:
        return []
    
    log("提取JULI频道，分组改为HK...")
    
    # 解析M3U内容
    lines = content.split('\n')
    channels = []
    current_extinf = None
    
    for line in lines:
        line = line.strip()
        if not line:
            continue
        
        if line.startswith('#EXTINF:'):
            current_extinf = line
        elif current_extinf and '://' in line and not line.startswith('#'):
            channels.append((current_extinf, line))
            current_extinf = None
    
    # 过滤JULI频道
    hk_channels = []
    
    for extinf, url in channels:
        if 'JULI' in extinf.upper():
            # 重命名为HK分组
            new_extinf = re.sub(r'JULI', 'HK', extinf, flags=re.IGNORECASE)
            
            # 确保group-title为HK
            if 'group-title=' in new_extinf:
                new_extinf = re.sub(r'group-title="[^"]*"', 'group-title="HK"', new_extinf)
            else:
                # 添加group-title
                if ',' in new_extinf:
                    parts = new_extinf.split(',', 1)
                    new_extinf = f'{parts[0]} group-title="HK",{parts[1]}'
            
            hk_channels.append((new_extinf, url))
    
    log(f"✅ 提取到 {len(hk_channels)} 个HK频道")
    
    return hk_channels

def extract_tw_channels(content):
    """提取4gtv前30个直播，分组改为TW"""
    if not content:
        return []
    
    log("提取4gtv前30个直播，分组改为TW...")
    
    # 解析M3U内容
    lines = content.split('\n')
    channels = []
    current_extinf = None
    
    for line in lines:
        line = line.strip()
        if not line:
            continue
        
        if line.startswith('#EXTINF:'):
            current_extinf = line
        elif current_extinf and '://' in line and not line.startswith('#'):
            channels.append((current_extinf, line))
            current_extinf = None
    
    # 过滤4gtv频道
    tw_channels = []
    
    for extinf, url in channels:
        if '4gtv' in extinf.lower():
            # 检查是否在黑名单中
            skip = False
            for black_word in BLACKLIST_TW:
                if black_word.lower() in extinf.lower():
                    skip = True
                    break
            
            if not skip:
                # 重命名为TW分组
                new_extinf = re.sub(r'4gtv', 'TW', extinf, flags=re.IGNORECASE)
                
                # 确保group-title为TW
                if 'group-title=' in new_extinf:
                    new_extinf = re.sub(r'group-title="[^"]*"', 'group-title="TW"', new_extinf)
                else:
                    # 添加group-title
                    if ',' in new_extinf:
                        parts = new_extinf.split(',', 1)
                        new_extinf = f'{parts[0]} group-title="TW",{parts[1]}'
                
                tw_channels.append((new_extinf, url))
    
    # 只取前30个
    tw_channels = tw_channels[:30]
    
    log(f"✅ 提取到 {len(tw_channels)} 个TW频道")
    
    return tw_channels

def sort_hk_channels_by_priority(hk_channels):
    """按指定顺序排序HK频道"""
    log("按指定顺序排序HK频道...")
    
    # 创建优先频道列表
    priority_channels = []
    other_channels = []
    
    for extinf, url in hk_channels:
        channel_name = extinf.split(',', 1)[1] if ',' in extinf else extinf
        
        # 检查是否是优先频道
        is_priority = False
        for priority in HK_PRIORITY_ORDER:
            if priority in channel_name:
                priority_channels.append((priority, extinf, url, channel_name))
                is_priority = True
                break
        
        if not is_priority:
            other_channels.append((extinf, url, channel_name))
    
    # 按优先顺序排序
    def get_priority_index(item):
        for i, priority in enumerate(HK_PRIORITY_ORDER):
            if priority == item[0]:
                return i
        return len(HK_PRIORITY_ORDER)
    
    priority_channels.sort(key=get_priority_index)
    
    # 合并结果
    sorted_channels = []
    
    # 添加优先频道
    for _, extinf, url, _ in priority_channels:
        sorted_channels.append((extinf, url))
    
    # 添加其他频道
    for extinf, url, _ in other_channels:
        sorted_channels.append((extinf, url))
    
    log(f"✅ 排序完成: {len(priority_channels)} 个优先频道 + {len(other_channels)} 个其他频道")
    
    return sorted_channels

def main():
    """主函数"""
    log("开始合并M3U文件...")
    
    # 1. 下载BB.m3u并获取EPG
    bb_content, epg_url = download_bb_m3u()
    if not bb_content:
        log("❌ 无法继续，BB.m3u下载失败")
        return
    
    # 2. 从代理获取内容
    proxy_content = get_content_from_proxy()
    
    # 3. 提取HK频道
    hk_channels_raw = []
    if proxy_content:
        hk_channels_raw = extract_hk_channels(proxy_content)
    else:
        log("⚠️  无法从代理获取内容，跳过HK频道")
    
    # 4. 排序HK频道
    hk_channels = sort_hk_channels_by_priority(hk_channels_raw) if hk_channels_raw else []
    
    # 5. 提取TW频道
    tw_channels = []
    if proxy_content:
        tw_channels = extract_tw_channels(proxy_content)
    else:
        log("⚠️  无法从代理获取内容，跳过TW频道")
    
    # 6. 构建M3U内容
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # M3U头部
    if epg_url:
        m3u_header = f'#EXTM3U url-tvg="{epg_url}"\n'
    else:
        m3u_header = '#EXTM3U\n'
    
    output = m3u_header + f"""# 自动合并 M3U 文件
# 生成时间: {timestamp} (北京时间)
# 下次更新: 每天 06:00 和 18:00 (北京时间)
# BB源: {BB_URL}
# 代理源: {CLOUDFLARE_PROXY}
# JULI分组已改为HK (按指定顺序排列在最前面)
# 4gtv分组已改为TW (前30个，排在后面，已过滤指定频道)
# EPG源: {epg_url if epg_url else 'BB的XML'}
# GitHub Actions 自动生成

"""
    
    # 添加BB内容（跳过第一行）
    bb_lines = bb_content.split('\n')
    bb_count = 0
    skip_first = True
    
    for line in bb_lines:
        line = line.rstrip()
        if not line:
            continue
        
        if skip_first and line.startswith('#EXTM3U'):
            skip_first = False
            continue
        
        output += line + '\n'
        if line.startswith('#EXTINF:'):
            bb_count += 1
    
    # 添加HK频道
    if hk_channels:
        output += f"\n# HK频道 (原JULI，按指定顺序排列在最前面)\n"
        output += f"# 优先顺序: {', '.join(HK_PRIORITY_ORDER)}\n"
        
        # 显示优先频道
        priority_added = False
        for channel_type in HK_PRIORITY_ORDER:
            type_channels = [(extinf, url) for extinf, url in hk_channels if channel_type in extinf]
            if type_channels:
                if not priority_added:
                    output += f"# --- 优先频道 ---\n"
                    priority_added = True
                
                for extinf, url in type_channels:
                    output += extinf + '\n'
                    output += url + '\n'
        
        # 显示其他HK频道
        other_channels = [(extinf, url) for extinf, url in hk_channels 
                         if not any(priority in extinf for priority in HK_PRIORITY_ORDER)]
        
        if other_channels:
            output += f"# --- 其他HK频道 ---\n"
            for extinf, url in other_channels:
                output += extinf + '\n'
                output += url + '\n'
    
    # 添加TW频道
    if tw_channels:
        output += f"\n# TW频道 (原4gtv，前30个，已过滤指定频道，排在HK之后)\n"
        output += f"# 已过滤: {', '.join(BLACKLIST_TW)}\n"
        for extinf, url in tw_channels:
            output += extinf + '\n'
            output += url + '\n'
    
    # 添加EPG使用说明
    output += f"""
# EPG使用说明
# 1. 此文件使用BB.m3u的EPG源: {epg_url}
# 2. 如果EPG不显示，可能是因为:
#    - EPG源不包含HK/TW频道的节目信息
#    - 播放器EPG缓存问题（尝试清除缓存）
#    - 时区设置问题（建议使用UTC+8）
# 3. 对于EPG问题，可以:
#    - 使用其他EPG源手动添加到播放器
#    - 在播放器中单独为HK/TW频道设置EPG

# 统计信息
# BB 频道数: {bb_count}
# HK 频道数: {len(hk_channels)} (按指定顺序排列)
# TW 频道数: {len(tw_channels)} (前30个，已过滤)
# 总频道数: {bb_count + len(hk_channels) + len(tw_channels)}
# EPG源: {epg_url if epg_url else '无'}
# 更新时间: {timestamp}
# 更新频率: 每天 06:00 和 18:00 (北京时间)
"""
    
    # 7. 保存文件
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(output)
    
    log(f"\n🎉 合并完成!")
    log(f"📁 文件: {OUTPUT_FILE}")
    log(f"📏 大小: {len(output)} 字符")
    log(f"📡 EPG: {epg_url}")
    log(f"📺 BB频道: {bb_count}")
    log(f"📺 HK频道: {len(hk_channels)} (按指定顺序)")
    log(f"📺 TW频道: {len(tw_channels)} (前30个，已过滤)")
    log(f"📺 总计: {bb_count + len(hk_channels) + len(tw_channels)}")
    log(f"🕒 下次自动更新: 北京时间 06:00 和 18:00")
    
    # EPG问题说明
    if epg_url:
        log(f"\n⚠️  EPG注意事项:")
        log(f"   使用的EPG源可能不包含HK/TW频道的节目信息")
        log(f"   这是原BB.m3u的EPG，主要覆盖BB频道")
        log(f"   如需HK/TW的EPG，需要另外寻找匹配的EPG源")

if __name__ == "__main__":
    main()
